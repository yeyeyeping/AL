...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, index):
        mask_path = str(
            self.images[index].parent.parent / "mask" / self.images[index].name)
...
class ACDCDataset2d(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data, label = np.asarray(data, dtype=np.float32), np.asarray(
...
class ACDCDataset3d(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 强度标准化
        img_obj = np.asarray((img_obj - img_obj.min()) /
                             (img_obj.max() - img_obj.min()), dtype=np.float32)
        # slice标准化
