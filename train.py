...
def save_query_state(dataloader, out_dir):
    labed, unlab = dataloader["labeled"], dataloader["unlabeled"]
    labed_ind, unlab_ind = sorted(
        labed.sampler.indices), sorted(unlab.sampler.indices)
    with open(join(out_dir, "query_state"), "a+") as fp:
        fp.write(str(labed_ind))
        fp.write("\n")

        fp.write(str(unlab_ind))
        fp.write("\n")
...
def get_trainer(config, dataloader, logger):
    ...
    trainer_obj = trainer_class(
        config, logger=logger, **strategy_section["trainer"]["additional_param"])

    query_strategy = query_strategy if "class" not in strategy_section else strategy_section[
        "class"]

    strategy_class = getclass(strategy_section["module"], query_strategy)
    strategy_obj = strategy_class(
        dataloader, trainer=trainer_obj, **strategy_section["additional_param"])
    logger.info(
        f"strategy:{type(strategy_obj)}  param:{strategy_section['additional_param']}")

    logger.info(
        f"trainer:{type(trainer_obj)} param: {strategy_section['trainer']['additional_param']}")
...
def main(config):
    ...
    save_query_state(dataloader, config["Training"]["output_dir"])
    cycle = 0
    while budget > 10:
        ...
        trainer.recover_from_best(cycle)
        ...
        dice_list.append(np.round(val_metric['avg_fg_dice'], 4))
        save_query_plot(config["Training"]["output_dir"],
                        labeled_percent, dice_list)
